...
let rDown = remote.filter((element, index) => {
  return index % 2 === 0;
});
let rUp = remote.filter((element, index) => {
  return index % 2 === 1;
});
...
//log(rDown, rUp);
//log(downKeys, upKeys);
var keyState = downKeys.map((element, index) => {
  return { down: element, up: upKeys[index] };
});
...
//window.addEventListener("keydown", (evt) => {
//if (downKeys.includes(k/*evt.key*/)) {
var pressed = keyState.find((element) => {
  return element.isPressed === true;
});
